# -*- coding: utf-8 -*-
"""ANVIL CAD Login Scheduler — separate module to avoid exec() GC issues in InitGui.py."""
...
def _show_login():
    try:
        ...
        if mw is None or not mw.isVisible():
            ...
            if _login_retries[0] <= 20:
                FreeCAD.Console.PrintLog(
                    f"ANVIL CAD: Main window not ready, retry {_login_retries[0]}/20\n"
                )
                QtCore.QTimer.singleShot(500, _show_login)
                return
            FreeCAD.Console.PrintWarning("ANVIL CAD: Main window not ready after retries, showing anyway\n")

        FreeCAD.Console.PrintMessage("ANVIL CAD: Login timer fired\n")
        ...
        import traceback
        FreeCAD.Console.PrintError(f"ANVIL CAD: Login dialog error: {exc}\n")
...
def schedule():
    ...
    FreeCAD.Console.PrintLog("ANVIL CAD: Login dialog scheduled (singleShot 2s)\n")
