...
class DefaultJoint:
    """Constrains a component's origin to the assembly origin (Creo-style default constraint).

    Uses ObjectToGround so the C++ solver keeps the part grounded, but the command
    first snaps the component placement to App.Placement() (identity = assembly origin).
    """

    def __init__(self, joint, obj_to_default):
        joint.Proxy = self
        self.joint = joint

        self.createObjectToGroundProperty(joint, obj_to_default)

    def createObjectToGroundProperty(self, joint, obj_to_default):
        joint.addProperty(
            "App::PropertyLinkGlobal",
            "ObjectToGround",
            "Ground",
            QT_TRANSLATE_NOOP("App::Property", "The object to ground at the assembly origin"),
            locked=True,
        )
        joint.ObjectToGround = obj_to_default
    ...
    def execute(self, fp):
        """Enforce identity placement (assembly origin) on the constrained part."""
        if fp.ObjectToGround:
            fp.ObjectToGround.Placement = App.Placement()


class ViewProviderDefaultJoint:
    """View provider for the Default (origin-to-origin) joint."""

    def __init__(self, obj):
        obj.Proxy = self

    def attach(self, vobj):
        """Minimal attach — no in-viewport glyph needed; the tree icon is sufficient."""
    ...
    def getIcon(self):
        ...
        icon_path = os.path.join(
            App.getHomePath(), "Mod", "Assembly", "Resources", "icons", "Assembly_CreateJointDefault.svg"
        )
...
class TaskAssemblyCreateJoint(QtCore.QObject):
    ...
    def accept(self):
        if self.jType == "Default":
            if not self.refs:
                App.Console.PrintWarning(
                    translate("Assembly", "Select a component to apply the Default Joint.\n")
                )
                return False

            moving_part = self.getMovingPart(self.refs[0])

            # Convert the placeholder joint in-place — it is already inside the
            # joint_group, so we just replace its proxy and rename it.
            # Never delete and recreate: doing so after deactivate() risks placing
            # the new object outside the Joints folder.
            self.joint.Label = "DefaultJoint"
            DefaultJoint(self.joint, moving_part)
            ViewProviderDefaultJoint(self.joint.ViewObject)

            # Snap the component to the assembly origin
            if moving_part:
    ...
    def handleInitialSelection(self):
        ...
        # do not accept initial selection if we don't have enough selected features
        needed = 1 if self.jType == "Default" else 2
        if len(self.refs) < needed:
            self.refs.clear()
            Gui.Selection.clearSelection()
        else:
            self.refs = self.refs[:needed]
            self.updateJoint()
    ...
    def moveMouse(self, info):
        max_refs = 1 if self.jType == "Default" else 2
        if len(self.refs) >= max_refs or (
            len(self.refs) == 1
            and (
                not self.presel_ref
    ...
    def addSelection(self, doc_name, obj_name, sub_name, mousePos):
        ...
        if len(self.refs) >= max_refs:
            # No more elements can be selected.
            acceptable = False
