...
class AssemblyWorkbench(Workbench):
    ...
    def setWatchers(self):
        ...
        def _refresh_active_subassembly_bold():
            """Bold the tree item matching the active subassembly; unbold all others.

            On every tick we scan every Assembly::AssemblyLink (and App::Link to
            an AssemblyObject) across all open docs and forcibly unbold any whose
            label != target. This is the robust fix for the case where FreeCAD's
            native bold (from a prior setActiveObject) sticks on a parent link
            after the user activates a deeper child — relying on a single
            prev_label is not enough because once prev_label == target_label, the
            old approach stopped re-unbolding.
            """
            try:
                import UtilsAssembly as _UA
                cache = getattr(_UA, "_active_asm_link_target", None)
                ...
                # Collect labels of every assembly-link object across all docs.
                # These are the only items we manage — leave non-link items alone.
                link_labels = set()
                try:
                    for _d in FreeCAD.listDocuments().values():
                        for _o in _d.Objects:
                            try:
                                _is_asm_link = (
                                    _o.isDerivedFrom("Assembly::AssemblyLink")
                                    or getattr(_o, "TypeId", "") == "Assembly::AssemblyLink"
                                )
                                if not _is_asm_link and _o.isDerivedFrom("App::Link"):
                                    _lo = getattr(_o, "LinkedObject", None)
                                    if _lo is not None and (
                                        _lo.isDerivedFrom("Assembly::AssemblyObject")
                                        or getattr(_lo, "TypeId", "") == "Assembly::AssemblyObject"
                                    ):
                                        _is_asm_link = True
                                if _is_asm_link and _o.Label:
                                    link_labels.add(_o.Label)
                            except Exception:
                                pass
                except Exception:
                    pass
                ...
                _highlight_brush = QtGui.QBrush(QtGui.QColor(0, 160, 0, 100))
                    ...
                    for item in _all_items(tree):
                        try:
                            ...
                            if text not in link_labels:
                                continue  # not an assembly-link tree item
                            if target_label is not None and text == target_label:
                                # Bold + highlight the active target.
                                f = item.font(0)
                                if not f.bold():
                                    f.setBold(True)
                                    item.setFont(0, f)
                                item.setBackground(0, _highlight_brush)
                            else:
                                # Forcibly unbold + clear background on EVERY non-target
                                # assembly-link item, every tick. Beats FreeCAD's native
                                # bold from a stale setActiveObject if it sneaks back in.
                                f = item.font(0)
                                if f.bold():
                                    f.setBold(False)
                                    item.setFont(0, f)
                                item.setBackground(0, _empty_brush)
                        except Exception:
                            pass
                    ...
                    if cache is not None:
                        ...
                        if link_doc is not None and link_doc.getObject(tgt_obj) is not None:
                            ...
                            if _root_asm_active:
                                ...
                                # Check that the active document is part of the assembly
                                # chain: either the parent doc holding the link (tgt_doc),
                                # or the subassembly's own doc (linked_asm.Document).
                                # If the user switched to a completely unrelated doc
                                # (e.g. a newly created body's .prt file), clear the cache.
                                _doc_in_chain = False
                                try:
                                    _linked_asm_doc = getattr(
                                        getattr(linked_asm, "Document", None), "Name", None
                                    )
                                    if doc.Name in (tgt_doc, _linked_asm_doc):
                                        _doc_in_chain = True
                                    else:
                                        for _d_obj in doc.Objects:
                                            try:
                                                _lo = getattr(_d_obj, "LinkedObject", None)
                                                if _lo is not None:
                                                    _lo_doc = getattr(
                                                        getattr(_lo, "Document", None), "Name", None
                                                    )
                                                    if _lo_doc in (tgt_doc, _linked_asm_doc):
                                                        _doc_in_chain = True
                                                        break
                                            except Exception:
                                                pass
                                    ...
                                    pass
                                if not _doc_in_chain:
                                    _UA._active_asm_link_target = None  # user left assembly context
                                    # Fall through to normal active-link detection below
                                else:
                                    # Cache is live. For level-2+ (cached link lives
                                    # in a different doc than the active tab) we must
                                    # keep FreeCAD's native active assembly CLEARED —
                                    # otherwise a stale setActiveObject on the parent
                                    # link can re-bold the parent (e.g. '45') alongside
                                    # the child (e.g. '55').
                                    try:
                                        if doc.Name != tgt_doc:
                                            _current = active_view.getActiveObject("assembly")
