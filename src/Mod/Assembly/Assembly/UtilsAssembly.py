...
# Explicitly activated Assembly::AssemblyLink target.
# Set by _activate_assembly; bypasses setEdit/setActiveObject which crash when the
# linked document is dependency-loaded (no active 3D view in its Gui::Document).
_active_asm_link_target = None  # tuple (doc_name, obj_name, linked_asm) or None


def setActiveAsmLinkTarget(doc_name, obj_name, linked_asm):
    global _active_asm_link_target
    _active_asm_link_target = (doc_name, obj_name, linked_asm) if linked_asm else None


def clearActiveAsmLinkTarget():
    global _active_asm_link_target
    _active_asm_link_target = None
...
def activeAssembly():
    ...
    # Check explicitly activated Assembly::AssemblyLink first.
    # setEdit() is skipped for AssemblyLink (crashes when linked doc has no 3D view).
    # The cache is the authoritative source so activeAssembly() always returns the
    # linked AssemblyObject (not the link itself).
    # We do NOT require App.ActiveDocument to be the link's document — for nested
    # (level-2+) subassemblies the link lives in a parent document that may differ
    # from the current GUI tab.  We only check that the link still exists.
    if _active_asm_link_target is not None:
        try:
            tgt_doc, tgt_obj, linked_asm = _active_asm_link_target
            link_doc = App.getDocument(tgt_doc)
            # TypeId fallback covers cases where isDerivedFrom misbehaves on
            # cross-document linked objects in some FreeCAD builds.
            is_asm = False
            try:
                is_asm = linked_asm.isDerivedFrom("Assembly::AssemblyObject")
            except Exception:
                pass
            if not is_asm:
                is_asm = getattr(linked_asm, "TypeId", "") == "Assembly::AssemblyObject"
            if (
                link_doc is not None
                and link_doc.getObject(tgt_obj) is not None
                and is_asm
            ):
                return linked_asm
        except Exception:
            pass
        _active_asm_link_target = None  # stale — clear it

    active_assembly = activePartOrAssembly()
    if active_assembly is not None and (
        active_assembly.isDerivedFrom("App::Link")
        or active_assembly.isDerivedFrom("Assembly::AssemblyLink")
    ):
        linked = getattr(active_assembly, "LinkedObject", None)
        if linked is not None and linked.isDerivedFrom("Assembly::AssemblyObject"):
            try:
                if active_assembly.ViewObject.isInEditMode() or linked.ViewObject.isInEditMode():
                    return linked
            except Exception:
                pass
    ...
    if active_assembly is not None and active_assembly.isDerivedFrom("Assembly::AssemblyObject"):
        ...
        # For a cross-document linked assembly that was registered via
        # setActiveObject("assembly", ...) by the link-sync timer, the
        # assembly's ViewObject.isInEditMode() is True in its own document.
        # Accept it here without re-checking isInEditMode on the parent side.
        if active_assembly.Document.Name != Gui.ActiveDocument.Document.Name:
            return active_assembly

    # When the user triggers the built-in tree action "Active object" on an
    # App::Link to an external assembly, FreeCAD may fail before it registers
    # an active object in the parent view. The link itself can still enter edit
    # mode, so use that as a fallback activation signal.
    if App.ActiveDocument is not None:
        for obj in App.ActiveDocument.Objects:
            if not obj.isDerivedFrom("App::Link") and not obj.isDerivedFrom("Assembly::AssemblyLink"):
                continue
            linked = getattr(obj, "LinkedObject", None)
            if linked is None or not linked.isDerivedFrom("Assembly::AssemblyObject"):
                continue
            try:
