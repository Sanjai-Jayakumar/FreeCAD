...
def createDefaultJoint(obj):
    """Snap the component to the assembly origin and record a Default joint."""
    if not UtilsAssembly.activeAssembly():
        return

    Gui.addModule("UtilsAssembly")
    Gui.addModule("JointObject")
    # Snap component placement to identity (assembly origin)
    commands = (
        f'obj = App.ActiveDocument.getObject("{obj.Name}")\n'
        "import FreeCAD as App\n"
        ...
        "assembly = UtilsAssembly.activeAssembly()\n"
        "joint_group = UtilsAssembly.getJointGroup(assembly)\n"
        'joint = joint_group.newObject("App::FeaturePython", "DefaultJoint")\n'
        "JointObject.DefaultJoint(joint, obj)"
    )
    Gui.doCommand(commands)
    Gui.doCommandGui("JointObject.ViewProviderDefaultJoint(joint.ViewObject)")
    Gui.doCommand("UtilsAssembly.activeAssembly().Document.recompute()")
    return Gui.doCommandEval("joint")


class CommandCreateJointDefault:
    ...
    def GetResources(self):
        ...
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "Assembly_CreateJointDefault",
                "<p>Creates a Default joint that constrains the selected component's origin to the "
                "assembly origin, aligning all axes (X, Y, Z). This is equivalent to Creo's "
                "Default constraint and fixes all 6 degrees of freedom.</p>"
                "<p>Select one component inside the active assembly, then activate this command.</p>",
            ),
            "CmdType": "ForEdit",
        }
    ...
    def Activated(self):
        activateJoint(JointObject.JointTypes.index("Default"))
...
if App.GuiUp:
    ...
    Gui.addCommand("Assembly_CreateJointDefault", CommandCreateJointDefault())
